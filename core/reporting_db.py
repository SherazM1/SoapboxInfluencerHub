...
def get_report(report_id: str) -> dict[str, Any] | None:
    ...
    with get_connection() as conn:
        ...
        items = conn.execute(
            """
            SELECT platform, creator_handle, content_title, content_description,
                   live_url, image_url, sort_order
        ...
        for index, item in enumerate(content_items):
            conn.execute(
                ...
                INSERT INTO report_content_items (
                    ...
                    content_description, live_url, image_url, sort_order
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    report_id,
                    item["platform"].strip(),
                    item["creator_handle"].strip(),
                    item["content_title"].strip(),
